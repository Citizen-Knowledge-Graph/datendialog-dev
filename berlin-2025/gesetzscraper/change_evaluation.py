...
def extract_text_diff(new_text: str, old_text: str) -> tuple[str, str]:
    ...
    while (
        start < len(old_words)
        and start < len(new_words)
        and old_words[start] == new_words[start]
    ):
        start += 1
    ...
    while (
        end_old > start
        and end_new > start
        and old_words[end_old - 1] == new_words[end_new - 1]
    ):
        end_old -= 1
        end_new -= 1
...
def compare_law_versions(
    df_new: pd.DataFrame, df_old: pd.DataFrame, only_output_diff: bool = True
) -> None:
    ...
    for p in common_paragraphs:
        if (
            df_new.loc[df_new.paragraph_id == p, "text_content"].iloc[0]
            != df_old.loc[df.paragraph_id == p, "text_content"].iloc[0]
        ):
            ...
            if only_output_diff:
                new_text, old_text = extract_text_diff(
