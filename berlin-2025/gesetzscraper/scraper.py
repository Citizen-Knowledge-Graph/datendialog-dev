...
def scrape_law(url: str) -> tuple[pd.DataFrame, dict]:
    ...
    for child in root:
        ...
        if paragraph_id is None:  # Header row with no paragraph_id
            ...
            if (new_state_comment := child.find(".//standangabe[standtyp='Neuf']/standkommentar")) is not None:
                metadata["new_state_comment"] = new_state_comment.text
            if (current_state_comment := child.find(".//standangabe[standtyp='Stand']/standkommentar")) is not None:
        ...
        try:
            text_content = "\n".join(" ".join(p.itertext()) for p in child.findall(".//text/Content/P"))
